...
#define		XBYAK_THROW_ERROW(...)		__assume(0)
...
#define MIE_PACK(x, y, z, w) ((x) * 64 + (y) * 16 + (z) * 4 + (w))
	...
	namespace inner {
		...
		{
#ifdef XBYAK64
			if (!IsInInt32(x)) XBYAK_THROW_ERROW(ERR_OFFSET_IS_TOO_BIG);
#endif
...
#error "not supported"
			...
			if (p == MAP_FAILED) XBYAK_THROW_ERROW(ERR_CANT_ALLOC);
			assert(p);
		...
		void free(uint8* p)
		{
			...
			if (i == sizeList_.end()) XBYAK_THROW_ERROW(ERR_BAD_PARAMETER);
			if (munmap((void*)i->first, i->second) < 0) XBYAK_THROW_ERROW(ERR_MUNMAP);
			sizeList_.erase(i);
		}
	};
#endif
	...
	public:
		...
		MH_XB_CONSTEXPR int getIdx() const { return idx_ & (EXT8BIT - 1); }
		MH_XB_CONSTEXPR bool isNone() const { return kind_ == 0; }
		MH_XB_CONSTEXPR bool isMMX() const { return is(MMX); }
		MH_XB_CONSTEXPR bool isXMM() const { return is(XMM); }
		MH_XB_CONSTEXPR bool isYMM() const { return is(YMM); }
		MH_XB_CONSTEXPR bool isZMM() const { return is(ZMM); }
		MH_XB_CONSTEXPR bool isXMEM() const { return is(XMM | MEM); }
		MH_XB_CONSTEXPR bool isYMEM() const { return is(YMM | MEM); }
		MH_XB_CONSTEXPR bool isZMEM() const { return is(ZMM | MEM); }
		MH_XB_CONSTEXPR bool isOPMASK() const { return is(OPMASK); }
		MH_XB_CONSTEXPR bool isBNDREG() const { return is(BNDREG); }
		MH_XB_CONSTEXPR bool isREG(int bit = 0) const { return is(REG, bit); }
		MH_XB_CONSTEXPR bool isMEM(int bit = 0) const { return is(MEM, bit); }
		MH_XB_CONSTEXPR bool isFPU() const { return is(FPU); }
		MH_XB_CONSTEXPR bool isExt8bit() const { return (idx_ & EXT8BIT) != 0; }
		MH_XB_CONSTEXPR bool isExtIdx() const { return (getIdx() & 8) != 0; }
		MH_XB_CONSTEXPR bool isExtIdx2() const { return (getIdx() & 16) != 0; }
		MH_XB_CONSTEXPR bool hasEvex() const { return isZMM() || isExtIdx2() || getOpmaskIdx() || getRounding(); }
		MH_XB_CONSTEXPR bool hasRex() const { return isExt8bit() || isREG(64) || isExtIdx(); }
		MH_XB_CONSTEXPR bool hasZero() const { return zero_; }
		MH_XB_CONSTEXPR int getOpmaskIdx() const { return mask_; }
		MH_XB_CONSTEXPR int getRounding() const { return rounding_; }
		MH_XB_CONSTEXPR void setKind(Kind kind)
		{
		...
		MH_XB_CONSTEXPR void setBit(int bit);
		void setOpmaskIdx(int idx, bool /*ignore_idx0*/ = true)
		{
			if (mask_) XBYAK_THROW_ERROW(ERR_OPMASK_IS_ALREADY_SET);
			mask_ = idx;
		}
		MH_XB_CONSTEXPR void setRounding(int idx)
		{
			if (rounding_) XBYAK_THROW_ERROW(ERR_ROUNDING_IS_ALREADY_SET);
			rounding_ = idx;
		}
		MH_XB_CONSTEXPR void setZero() { zero_ = true; }
		// ah, ch, dh, bh?
		MH_XB_CONSTEXPR bool isHigh8bit() const
		{
		...
		MH_XB_CONSTEXPR bool is(int kind, uint32 bit = 0) const
		{
			return (kind == 0 || (kind_ & kind)) && (bit == 0 || (bit_ & bit)); // cf. you can set (8|16)
		}
		MH_XB_CONSTEXPR bool isBit(uint32 bit) const { return (bit_ & bit) != 0; }
		MH_XB_CONSTEXPR uint32 getBit() const { return bit_; }
		const char* toString() const
		{
			...
			XBYAK_THROW_ERROW(ERR_INTERNAL);
		}
		MH_XB_CONSTEXPR bool isEqualIfNotInherited(const Operand& rhs) const { return idx_ == rhs.idx_ && kind_ == rhs.kind_ && bit_ == rhs.bit_ && zero_ == rhs.zero_ && mask_ == rhs.mask_ && rounding_ == rhs.rounding_; }
		MH_XB_CONSTEXPR bool operator==(const Operand& rhs) const;
		MH_XB_CONSTEXPR bool operator!=(const Operand& rhs) const { return !operator==(rhs); }
		...
		inline void Operand::setBit(int bit)
	{
		...
		//ERR:
		XBYAK_THROW_ERROW(ERR_CANT_CONVERT);
	}
	...
	public:
		MH_XB_CONSTEXPR Reg() { }
		MH_XB_CONSTEXPR Reg(int idx, Kind kind, int bit = 0, bool ext8bit = false) : Operand(idx, kind, bit, ext8bit) { }
		// convert to Reg8/Reg16/Reg32/Reg64/XMM/YMM/ZMM
		MH_XB_CONSTEXPR Reg changeBit(int bit) const { Reg r(*this); r.setBit(bit); return r; }
		MH_XB_CONSTEXPR uint8 getRexW() const { return isREG(64) ? 8 : 0; }
		MH_XB_CONSTEXPR uint8 getRexR() const { return isExtIdx() ? 4 : 0; }
		MH_XB_CONSTEXPR uint8 getRexX() const { return isExtIdx() ? 2 : 0; }
		MH_XB_CONSTEXPR uint8 getRexB() const { return isExtIdx() ? 1 : 0; }
		MH_XB_CONSTEXPR uint8 getRex(const Reg& base = Reg()) const
		{
	...
	struct Reg8 : public Reg {
		MH_XB_CONSTEXPR explicit Reg8(int idx = 0, bool ext8bit = false) : Reg(idx, Operand::REG, 8, ext8bit) { }
	};

	struct Reg16 : public Reg {
		MH_XB_CONSTEXPR explicit Reg16(int idx = 0) : Reg(idx, Operand::REG, 16) { }
	};

	struct Mmx : public Reg {
		MH_XB_CONSTEXPR explicit Mmx(int idx = 0, Kind kind = Operand::MMX, int bit = 64) : Reg(idx, kind, bit) { }
	};
	...
	struct EvexModifierRounding {
		...
		MH_XB_CONSTEXPR explicit EvexModifierRounding(int rounding) : rounding(rounding) {}
		int rounding;
	};
	struct EvexModifierZero { EvexModifierZero() {} };

	struct Xmm : public Mmx {
		MH_XB_CONSTEXPR explicit Xmm(int idx = 0, Kind kind = Operand::XMM, int bit = 128) : Mmx(idx, kind, bit) { }
		MH_XB_CONSTEXPR Xmm(Kind kind, int idx) : Mmx(idx, kind, kind == XMM ? 128 : kind == YMM ? 256 : 512) { }
		MH_XB_CONSTEXPR Xmm operator|(const EvexModifierRounding& emr) const { Xmm r(*this); r.setRounding(emr.rounding); return r; }
		MH_XB_CONSTEXPR Xmm copyAndSetIdx(int idx) const { Xmm ret(*this); ret.setIdx(idx); return ret; }
		MH_XB_CONSTEXPR Xmm copyAndSetKind(Operand::Kind kind) const { Xmm ret(*this); ret.setKind(kind); return ret; }
	};

	struct Ymm : public Xmm {
		MH_XB_CONSTEXPR explicit Ymm(int idx = 0, Kind kind = Operand::YMM, int bit = 256) : Xmm(idx, kind, bit) { }
		MH_XB_CONSTEXPR Ymm operator|(const EvexModifierRounding& emr) const { Ymm r(*this); r.setRounding(emr.rounding); return r; }
	};

	struct Zmm : public Ymm {
		MH_XB_CONSTEXPR explicit Zmm(int idx = 0) : Ymm(idx, Operand::ZMM, 512) { }
		MH_XB_CONSTEXPR Zmm operator|(const EvexModifierRounding& emr) const { Zmm r(*this); r.setRounding(emr.rounding); return r; }
	};

	struct Opmask : public Reg {
		MH_XB_CONSTEXPR explicit Opmask(int idx = 0) : Reg(idx, Operand::OPMASK, 64) {}
	};

	struct BoundsReg : public Reg {
		MH_XB_CONSTEXPR explicit BoundsReg(int idx = 0) : Reg(idx, Operand::BNDREG, 128) {}
	};
	...
	struct Fpu : public Reg {
		MH_XB_CONSTEXPR explicit Fpu(int idx = 0) : Reg(idx, Operand::FPU, 32) { }
	};

	struct Reg32e : public Reg {
		MH_XB_CONSTEXPR explicit Reg32e(int idx, int bit) : Reg(idx, Operand::REG, bit) {}
	};
	struct Reg32 : public Reg32e {
		MH_XB_CONSTEXPR explicit Reg32(int idx = 0) : Reg32e(idx, 32) {}
	};
#ifdef XBYAK64
	struct Reg64 : public Reg32e {
		MH_XB_CONSTEXPR explicit Reg64(int idx = 0) : Reg32e(idx, 64) {}
	};
	struct RegRip {
		sint64 disp_;
		const Label* label_;
		bool isAddr_;
		MH_XB_CONSTEXPR explicit RegRip(sint64 disp = 0, const Label* label = 0, bool isAddr = false) : disp_(disp), label_(label), isAddr_(isAddr) {}
		...
		friend const RegRip operator+(const RegRip& r, const Label& label) {
			if (r.label_ || r.isAddr_) XBYAK_THROW_ERROW(ERR_BAD_ADDRESSING);
	...
	public:
		...
		MH_XB_CONSTEXPR explicit Segment(int idx) : idx_(idx) { assert(0 <= idx_ && idx_ < 6); }
		MH_XB_CONSTEXPR int getIdx() const { return idx_; }
		const char* toString() const
		{
	...
	public:
		...
		enum { i32e = 32 };
#endif
		MH_XB_CONSTEXPR RegExp(size_t disp = 0) : scale_(0), disp_(disp) { }
		MH_XB_CONSTEXPR RegExp(const Reg& r, int scale = 1)
			: scale_(scale)
			, disp_(0)
		{
			if (!r.isREG(i32e) && !r.is(Reg::XMM | Reg::YMM | Reg::ZMM)) XBYAK_THROW_ERROW(ERR_BAD_SIZE_OF_REGISTER);
			if (scale == 0) return;
			if (scale != 1 && scale != 2 && scale != 4 && scale != 8) XBYAK_THROW_ERROW(ERR_BAD_SCALE);
		...
		void verify() const
		{
			if (base_.getBit() >= 128) XBYAK_THROW_ERROW(ERR_BAD_SIZE_OF_REGISTER);
			if (index_.getBit() && index_.getBit() <= 64) {
				if (index_.getIdx() == Operand::ESP) XBYAK_THROW_ERROW(ERR_ESP_CANT_BE_INDEX);
				if (base_.getBit() && base_.getBit() != index_.getBit()) XBYAK_THROW_ERROW(ERR_BAD_SIZE_OF_REGISTER);
			}
		}
	...
	{
		if (a.index_.getBit() && b.index_.getBit()) XBYAK_THROW_ERROW(ERR_BAD_ADDRESSING);
		RegExp ret = a;
		if (!ret.index_.getBit()) { ret.index_ = b.index_; ret.scale_ = b.scale_; }
		if (b.base_.getBit()) {
			if (ret.base_.getBit()) {
				if (ret.index_.getBit()) XBYAK_THROW_ERROW(ERR_BAD_ADDRESSING);
	...
	public:
		...
		{
			if (maxSize_ > 0 && top_ == 0) XBYAK_THROW_ERROW(ERR_CANT_ALLOC);
			if ((type_ == ALLOC_BUF && userPtr != DontSetProtectRWE && useProtect()) && !setProtectMode(PROTECT_RWE, false)) {
				alloc_->free(top_);
				XBYAK_THROW_ERROW(ERR_CANT_PROTECT);
			}
		}
		virtual ~CodeArray()
		{
		...
		bool setProtectMode(ProtectMode mode, bool throwException = true)
		{
			...
			if (isOK) return true;
			if (throwException) XBYAK_THROW_ERROW(ERR_CANT_PROTECT);
			return false;
		}
		...
		void db(int code)
		{
			if (size_ >= maxSize_) {
				if (type_ == AUTO_GROW) {
					...
					XBYAK_THROW_ERROW(ERR_CODE_IS_TOO_BIG);
				}
			}
		...
		{
			if (codeSize > 8) XBYAK_THROW_ERROW(ERR_BAD_PARAMETER);
		...
		{
			if (size > maxSize_) XBYAK_THROW_ERROW(ERR_OFFSET_IS_TOO_BIG);
			size_ = size;
		}
		void dump() const
		{
		...
		*/
		void rewrite(size_t offset, uint64 disp, size_t size)
		{
			...
			if (size != 1 && size != 2 && size != 4 && size != 8) XBYAK_THROW_ERROW(ERR_BAD_PARAMETER);
	...
	class LabelManager {
		...
		void define_inner(DefList& defList, UndefList& undefList, const T& labelId, size_t addrOffset)
		{
			...
			if (!ret.second) XBYAK_THROW_ERROW(ERR_LABEL_IS_REDEFINED);
...
#ifdef XBYAK64
					if (jmp->jmpSize <= 4 && !inner::IsInInt32(disp)) XBYAK_THROW_ERROW(ERR_OFFSET_IS_TOO_BIG);
#endif
					if (jmp->jmpSize == 1 && !inner::IsInDisp8((uint32)disp)) XBYAK_THROW_ERROW(ERR_LABEL_IS_TOO_FAR);
				}
				if (base_->isAutoGrow()) {
	...
	public:
		...
		void leaveLocal()
		{
			if (stateList_.size() <= 2) XBYAK_THROW_ERROW(ERR_UNDER_LOCAL_LABEL);
			if (hasUndefinedLabel_inner(stateList_.back().undefList)) XBYAK_THROW_ERROW(ERR_LABEL_IS_NOT_FOUND);
			stateList_.pop_back();
		}
		void set(CodeArray* base) { base_ = base; }
		void defineSlabel(std::string label)
		{
			if (label == "@b" || label == "@f") XBYAK_THROW_ERROW(ERR_BAD_LABEL_STR);
			if (label == "@@") {
		...
		void assign(Label& dst, const Label& src)
		{
			...
			if (i == clabelDefList_.end()) XBYAK_THROW_ERROW(ERR_LABEL_ISNOT_SET_BY_L);
		...
		bool getOffset(size_t* offset, std::string& label) const
		{
			...
			if (label == "@b") {
				if (defList.find("@f") != defList.end()) {
					...
					XBYAK_THROW_ERROW(ERR_LABEL_IS_NOT_FOUND);
				}
			}
			else if (label == "@f") {
				if (defList.find("@f") != defList.end()) {
	...
	{
		if (id) XBYAK_THROW_ERROW(ERR_LABEL_IS_ALREADY_SET_BY_L);
		id = rhs.id;
		mgr = rhs.mgr;
		if (mgr) mgr->incRefCount(id, this);
		return *this;
	...
	private:
		...
		void rex(const Operand& op1, const Operand& op2 = Operand())
		{
			...
			if (p1->isMEM()) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
			if (p2->isMEM()) {
		...
		void vex(const Reg& reg, const Reg& base, const Operand* v, int type, int code, bool x = false)
		{
			...
			if ((idx | reg.getIdx() | base.getIdx()) >= 16) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		...
		{
			if (!(type & (T_EVEX | T_MUST_EVEX))) XBYAK_THROW_ERROW(ERR_EVEX_IS_INVALID);
		...
		void setSIB(const RegExp& e, int reg, int disp8N = 0)
		{
			...
			if (high != 0 && high != 0xFFFFFFFF) XBYAK_THROW_ERROW(ERR_OFFSET_IS_TOO_BIG);
#endif
		...
		{
			if (addr.is64bitDisp()) XBYAK_THROW_ERROW(ERR_CANT_USE_64BIT_DISP);
			rex(addr, reg);
		...
		{
			if (addr.is64bitDisp()) XBYAK_THROW_ERROW(ERR_CANT_USE_64BIT_DISP);
			if (reg.isBit(8)) XBYAK_THROW_ERROW(ERR_BAD_SIZE_OF_REGISTER);
			rex(addr, reg);
		...
		{
			if (addr.is64bitDisp()) XBYAK_THROW_ERROW(ERR_CANT_USE_64BIT_DISP);
			if (addr.getMode() != Address::M_ModRM) XBYAK_THROW_ERROW(ERR_INVALID_MIB_ADDRESS);
			if (BIT == 64 && addr.is32bit()) db(0x67);
		...
		void makeJmp(uint32 disp, LabelType type, uint8 shortCode, uint8 longCode, uint8 longPref)
		{
			...
			if (type != T_NEAR && inner::IsInDisp8(disp - shortJmpSize)) {
				...
				if (type == T_SHORT) XBYAK_THROW_ERROW(ERR_LABEL_IS_TOO_FAR);
		...
		{
			if (isAutoGrow()) {
				if (!isNEAR(type)) XBYAK_THROW_ERROW(ERR_ONLY_T_NEAR_IS_SUPPORTED_IN_AUTO_GROW);
		...
		{
			if (!permitVisb && addr.isVsib()) XBYAK_THROW_ERROW(ERR_BAD_VSIB_ADDRESSING);
			if (addr.getMode() == Address::M_ModRM) {
				...
				if (addr.getLabel()) { // [rip + Label]
					...
					if (addr.getMode() == Address::M_ripAddr) {
						if (isAutoGrow()) XBYAK_THROW_ERROW(ERR_INVALID_RIP_IN_AUTO_GROW);
		...
		{
			if (isValid && !isValid(reg, op)) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		...
		void opMovXMM(const Operand& op1, const Operand& op2, int code, int pref)
		{
			...
			if (op1.isXMM() && op2.isMEM()) {
				...
				XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
			}
		}
		...
		{
			if (_cl.getIdx() != Operand::CL) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		...
		{
			if (_cl && _cl->getIdx() != Operand::CL) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		...
		void opRM_I(const Operand& op, uint32 imm, int code, int ext)
		{
			...
			if (op.getBit() < immBit) XBYAK_THROW_ERROW(ERR_IMM_IS_TOO_BIG);
		...
		{
			if (op.isMEM() && op.getBit() == 0) XBYAK_THROW_ERROW(ERR_MEM_SIZE_IS_NOT_SPECIFIED);
		}
		/*
		...
		*/
		...
		{
			if (op.isBit(32)) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
			int w = op.isBit(16);
#ifdef XBYAK64
			if (op.isHigh8bit()) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
#endif
		...
		void opFpuMem(const Address& addr, uint8 m16, uint8 m32, uint8 m64, uint8 ext, uint8 m64ext)
		{
			...
			if (!code) XBYAK_THROW_ERROW(ERR_BAD_MEM_SIZE);
		...
		void opFpuFpu(const Fpu& reg1, const Fpu& reg2, uint32 code1, uint32 code2)
		{
			...
			if (!code) XBYAK_THROW_ERROW(ERR_BAD_ST_COMBINATION);
			db(uint8(code >> 8));
		...
		void opVex(const Reg& r, const Operand* p1, const Operand& op2, int type, int code, int imm8 = NONE)
		{
			if (op2.isMEM()) {
				...
				if ((type & (T_MUST_EVEX | T_MEM_EVEX)) || r.hasEvex() || (p1 && p1->hasEvex()) || addr.isBroadcast() || addr.getOpmaskIdx()) {
					...
					if (aaa && !(type & T_M_K)) XBYAK_THROW_ERROW(ERR_INVALID_OPMASK_WITH_MEMORY);
					bool b = false;
					if (addr.isBroadcast()) {
						if (!(type & (T_B32 | T_B64))) XBYAK_THROW_ERROW(ERR_INVALID_BROADCAST);
						b = true;
					}
		...
		void opGpr(const Reg32e& r, const Operand& op1, const Operand& op2, int type, uint8 code, bool isR_R_RM, int imm8 = NONE)
		{
			...
			if (p1->getBit() != bit || (p2->isREG() && p2->getBit() != bit)) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		...
		void opAVX_X_X_XM(const Xmm& x1, const Operand& op1, const Operand& op2, int type, int code0, int imm8 = NONE)
		{
			...
			// (x1, x2, op)
			if (!((x1.isXMM() && x2->isXMM()) || ((type & T_YMM) && ((x1.isYMM() && x2->isYMM()) || (x1.isZMM() && x2->isZMM()))))) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		...
		{
			if (!op3.isMEM() && (x2.getKind() != op3.getKind())) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
			opVex(k, &x2, op3, type, code0, imm8);
		}
		// (x, x/m), (y, x/m256), (z, y/m)
		void checkCvt1(const Operand& x, const Operand& op) const
		{
			if (!op.isMEM() && !(x.is(Operand::XMM | Operand::YMM) && op.isXMM()) && !(x.isZMM() && op.isYMM())) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		}
		// (x, x/m), (x, y/m256), (y, z/m)
		void checkCvt2(const Xmm& x, const Operand& op) const
		{
			if (!(x.isXMM() && op.is(Operand::XMM | Operand::YMM | Operand::MEM)) && !(x.isYMM() && op.is(Operand::ZMM | Operand::MEM))) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		}
		...
		{
			if (!(x1.isXMM() && x2.isXMM() && (op.isREG(i32e) || op.isMEM()))) XBYAK_THROW_ERROW(ERR_BAD_SIZE_OF_REGISTER);
			Xmm x(op.getIdx());
		...
		void opSp1(const Reg& reg, const Operand& op, uint8 pref, uint8 code0, uint8 code1)
		{
			...
			if (!is16bit && !(reg.isREG(i32e) && (op.isREG(reg.getBit()) || op.isMEM()))) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
			if (is16bit) db(0x66);
		...
		{
			const RegExp& regExp = addr.getRegExp();
			if (!regExp.isVsib(128 | 256)) XBYAK_THROW_ERROW(ERR_BAD_VSIB_ADDRESSING);
			const int y_vx_y = 0;
			const int y_vy_y = 1;
			//		const int x_vy_x = 2;
			...
			if (!x1.isXMM() || isAddrYMM || !x2.isXMM()) {
				...
				if (!isOK) XBYAK_THROW_ERROW(ERR_BAD_VSIB_ADDRESSING);
			}
		...
		void checkGather2(const Xmm& x1, const Reg& x2, int mode) const
		{
			...
			XBYAK_THROW_ERROW(ERR_BAD_VSIB_ADDRESSING);
		}
		void opGather2(const Xmm& x, const Address& addr, int type, uint8 code, int mode)
		{
			if (x.hasZero()) XBYAK_THROW_ERROW(ERR_INVALID_ZERO);
		...
		*/
		...
		{
			if (mode) {
				if (!op.isMEM() && !((op.isXMM() && x.isXMM()) || (op.isXMM() && x.isYMM()) || (op.isYMM() && x.isZMM())))  XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
			}
			else {
				if (!op.isMEM() && !op.isXMM()) XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
			}
			opVex(x, 0, op, type, code);
		}
		void opGatherFetch(const Address& addr, const Xmm& x, int type, uint8 code, Operand::Kind kind)
		{
			if (addr.hasZero()) XBYAK_THROW_ERROW(ERR_INVALID_ZERO);
			if (addr.getRegExp().getIndex().getKind() != kind) XBYAK_THROW_ERROW(ERR_BAD_VSIB_ADDRESSING);
			opVex(x, 0, addr, type, code);
		}
	...
	public:
		...
		*/
		...
		static constexpr AddressFrame ptr{ 0 }, byte{ 8 }, word{ 16 }, dword{ 32 }, qword{ 64 }, xword{ 128 }, yword{ 256 }, zword{ 512 };
	...
	public:
		...
		*/
		...
		void mov(const Operand& op, size_t imm)
		{
			if (op.isREG()) {
				...
				if (immSize <= 4) {
					...
					if (s != 0 && s != -1) XBYAK_THROW_ERROW(ERR_IMM_IS_TOO_BIG);
				}
				else {
					if (!inner::IsInInt32(imm)) XBYAK_THROW_ERROW(ERR_IMM_IS_TOO_BIG);
					immSize = 4;
				}
		...
		void pop(const Segment& seg)
		{
			...
			case Segment::cs: XBYAK_THROW_ERROW(ERR_BAD_COMBINATION);
		...
		*/
		...
		{
			if (hasUndefinedLabel()) XBYAK_THROW_ERROW(ERR_LABEL_IS_NOT_FOUND);
			if (isAutoGrow()) {
				calcJmpAddress();
...
#include "xbyak_mnemonic.h"
		...
		*/
		...
		{
			if (x == 1) return;
			if (x < 1 || (x & (x - 1))) XBYAK_THROW_ERROW(ERR_BAD_ALIGN);
