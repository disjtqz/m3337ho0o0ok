...
void* locate_challenge_entity() {
	...
	if (!etable)
		return nullptr;
	idEventArg tmp;
	tmp.make_string("idGameChallenge");

	void* callev = get_identity_callevent_impl();

	idEventArg res; 
	idEventDef* iscl = ev_isClass.Get();
...
/*	void* callev = VTBL_MEMBER(idEntity, VTBLOFFS_CALLEVENT)::Get();
	...
	call_as<void>(callev, self, out_arg, tdef_name, args);*/
		call_as<void>(callev, p, &res, iscl, &tmp);

		if ((res.value.i&0xff) != 0) {
			return p;
		}
	}
	return nullptr;
...
*/
bool toggle_idplayer_boolean(void* player, const char* property_name, bool use_explicit_value, bool explicit_value, bool silent) {

	auto headattr = idType::FindClassField("idPlayer", property_name);
	if (!headattr) {
		if(!silent) //honor silent even for errors
...
void mh_ScriptCmdEnt_idEntity_void(idEventDef* MH_NOESCAPE tdef_name, void* MH_NOESCAPE self, idEventArg* MH_NOESCAPE args) {
	...
	call_as<void>(get_identity_callevent_impl(), self, &g_sink_eventarg_ret, tdef_name, args);
}
